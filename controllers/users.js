...
export const postCreateUserEntry = async (req,res) => {
    ...
    try {
      let newUserInDB = await db.one('INSERT INTO users VALUES(${id}, ${username}, ${email}, ${password}) RETURNING *', {...newUser, id: newId, password: pwHash});
...
try {
  //get info from db
  let userFromDb = await db.one('SELECT * FROM users WHERE username = $1', user.username)
  //if no user matchs
  if(userFromDb === null ) {
